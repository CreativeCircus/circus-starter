		...
		axios
			...
			.then((response) => {
				if (parsedPackage.version === response.data.version) {
					...
					console.warn(
						`If this is a new project, get a new copy. If it's an old project, consider updating for new gulpy goodness.`.red.bold.inverse
					);
				}
				done();
			})
...
gulp.task('default', gulp.series('welcome', 'start-browsersync', 'build', 'watch', 'make-cool-shit', 'version'));
...
gulp.task(
	'no-browser-sync',
	gulp.series('welcome', gulp.parallel('html-refresh', 'html-check', 'image-compress', 'js-compile', 'sass-compile'), 'make-cool-shit', 'version')
);
